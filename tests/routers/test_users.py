...
async def test_change_password_others_missing(auth_client):
    response = await auth_client.put('/users/2', json={'old_password': 'mper123', 'new_password': 'mper321'})
...
async def test_change_password_others(client, auth_client):
    ...
    await client.post('/auth/login', data={'username': 'mper@mper.com', 'password': 'mper'})
    response = await auth_client.put('/users/2', json={'old_password': 'mper', 'new_password': '123'})
    assert response.status_code == 403
    assert response.json() == {"detail": "Not authorized"}


async def test_change_password_self_bad(auth_client):
    response = await auth_client.put('/users/1', json={'old_password': 'mper123', 'new_password': 'mper321'})
    assert response.status_code == 401
    assert response.json() == {"detail": "Incorrect old password"}


async def test_change_password_self_good(auth_client):
    response = await auth_client.put('/users/1', json={'old_password': 'password123', 'new_password': 'mper321'})
